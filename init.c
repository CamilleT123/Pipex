...
/*   Updated: 2024/02/27 17:59:47 by ctruchot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// writes what is sent to the stdin until the LIMITE signal
...
int	get_here_doc(char **av)
{
	...
	if (fd == -1)
		return (ft_putstr_fd(strerror(errno), 2), free(lim), 1);
	line = get_next_line(0);
	if (line == NULL)
		return (free(lim), close(fd));
...
int	struct_init(int ac, t_struc *data, bool here_doc)
{
	...
	if (!data->pid)
		return (ft_putstr_fd(strerror(errno), 2), clean_exit_parent(data), 1);
	return (0);
}
